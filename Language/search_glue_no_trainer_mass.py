...
def parse_args():
    ...
    parser.add_argument('--expert_repeat', type=int, default=8)
    parser.add_argument('--add_expert_size', type=int, default=0)
    parser.add_argument('--key_gate', action='store_true')
    parser.add_argument('--gate_type', type=str, default='mass', choices=['mass', 'top', 'cosine_top', 'gated_multi_gate', 'gated_multi_gate_t', 'gated_multi_gate_custom', 'gha_mix', 'rand'])
    ...
    parser.add_argument('--rm_threshold', type=float, default=0.90, help='Redundancy measurement threshold for MASS')
    parser.add_argument('--mass_warmup_steps', type=int, default=10000, help='Warmup steps before starting MASS expansion')
    parser.add_argument('--mass_window_size', type=int, default=10000, help='Window size for MASS statistics collection')
    ...
    if args.expansion_start_ratio >= args.expansion_end_ratio:
        raise ValueError(f"expansion_start_ratio ({args.expansion_start_ratio}) must be less than expansion_end_ratio ({args.expansion_end_ratio})")
...
def main(args, seed=0, lr=3e-5):
            ...
            with open(os.path.join(output_dir, 'done'), 'r') as f:
                ...
                for file in files:
                    if 'Final_K' in file:
                        ...
                        match = re.match(r'Final_K_(\d+)_Avg_k_([\d\.]+)\.txt', base)
                        if match:
                            final_K = int(match.group(1))
                            final_avg_k = float(match.group(2))
                            break
                return best_eval_metric, average_metric(best_eval_metric), final_K, final_avg_k
    ...
    if args.to_MoE:
        from moe_utils_mass import bert_to_MoE
    ...
    best_eval_metric = {}
    all_eval_results = {}
    expansion_start_step = int(args.max_train_steps * args.expansion_start_ratio)
    expansion_stop_step = int(args.max_train_steps * args.expansion_end_ratio)
    global_start_expansion = False
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            accelerator.backward(task_loss/args.gradient_accumulation_steps)

            # MASS expansion
            if 'bert' in args.model_name_or_path and not global_stop_expansion:
                layers = model.module.bert.encoder.layer if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.bert.encoder.layer
                
                if not global_start_expansion and completed_steps >= expansion_start_step:
                    global_start_expansion = True
                    # reset MASS parameters before training
                    for i, layer in enumerate(layers):
                        if i in args.moe_layers:
                            layer.mlp.t_warmup = max(30, int(args.max_train_steps*0.05))
                            layer.mlp.window_size = max(60, int(args.max_train_steps*0.09))
                            layer.mlp._init_mass_tracking()
                            layer.mlp._register_gradient_hooks()
                            print(f"MASS parameters reset for layer {i} with t_warmup {layer.mlp.t_warmup} and window_size {layer.mlp.window_size}")
                            logger.info(f"MASS parameters reset for layer {i} with t_warmup {layer.mlp.t_warmup} and window_size {layer.mlp.window_size}")
                    print(f"Starting expert expansion at step {completed_steps} ({args.expansion_start_ratio*100:.1f}% of training)")
                
                if global_start_expansion:
                    for i, layer in enumerate(layers):
                        if i in args.moe_layers:   
                            expansion = False
                            info = layer.mlp._check_expansion_conditions(completed_steps)
                            if info:
                                last_expert_idx = layer.mlp.num_global_experts - 1
                                if layer.mlp.expansion_count > 1:
                                    with torch.no_grad():
                                        layer.mlp.gates[0].experts_mask[last_expert_idx] = 0.0
                                        outputs_mask = model(**batch)
                                        task_loss_mask = outputs_mask.loss
                                        ll_gap = (task_loss_mask - task_loss).detach().cpu()
                                        
                                        if ll_gap <= 0.0:
                                            if layer.mlp.expansion_patience:
                                                layer.mlp.expansion_patience -= 1
                                                layer.mlp._remove_expert()
                                                expansion = True
                                                print(f"No LL improvement with {ll_gap}, left patience {layer.mlp.expansion_patience}")
                                                logger.info(f"No LL improvement with {ll_gap}, left patience {layer.mlp.expansion_patience}")
                                            else:
                                                layer.mlp.stop_expansion = True
                                                print(f"Expansion stopped with final K={layer.mlp.num_global_experts}")
                                        else:
                                            layer.mlp.gates[0].experts_mask[last_expert_idx] = 1.0
                                            expansion = True
                                else:
                                    expansion = True

                            if expansion:
                                layer.mlp.expand_expert(info, completed_steps)
                                mass_info = layer.mlp.get_mass_info()
                                print(f"MASS at epoch {epoch}, step {completed_steps}, layer {i}: Active experts: {mass_info['active_experts']}/{mass_info['max_experts']}")
                                logger.info(f"MASS at epoch {epoch}, step {completed_steps}, layer {i}: Active experts: {mass_info['active_experts']}/{mass_info['max_experts']}")
                                
                                if layer.mlp.num_global_experts >= layer.mlp.max_num_global_experts:
                                    layer.mlp.stop_expansion = True
                                    print(f"Expansion stopped reaching maximum K={layer.mlp.max_num_global_experts}")
                                    logger.info(f"Expansion stopped reaching maximum K={layer.mlp.max_num_global_experts}")
                        ...
                        if 'bert' in args.model_name_or_path:
                            for i, layer in enumerate(layers):
                                if i in args.moe_layers and hasattr(layer.mlp, 'get_redundancy_loss'):
                                    redundancy_loss += layer.mlp.get_redundancy_loss() 

                    if redundancy_loss.item() > 0:
                        if args.with_tracking:
                            total_loss += redundancy_loss.detach().float()
                        redundancy_loss = redundancy_loss / args.gradient_accumulation_steps
                        accelerator.backward(redundancy_loss*args.mass_redundancy_weight)

                    # Check if we should stop expansion
                    if completed_steps+1 >= expansion_stop_step:
                        global_stop_expansion = True
                        print(f"Expansion phase completed at step {completed_steps+1} ({args.expansion_end_ratio*100:.1f}% of training). Removing MASS hooks...")
                        wrapped_model = model.module if hasattr(model, 'module') else model
                        layers = wrapped_model.bert.encoder.layer if 'bert' in args.model_name_or_path else wrapped_model.encoder.layer
                        for i, layer in enumerate(layers):
                            if i in args.moe_layers and hasattr(layer.mlp, 'disable_mass_tracking'):
        ...
        # Evaluation after main training epoch
        model.eval()
        samples_seen = 0
    ...
    if output_dir is not None:
        ...
        for i, layer in enumerate(layers):
            if i in args.moe_layers and hasattr(layer.mlp, 'get_mass_info'):
                mass_info = layer.mlp.get_mass_info()
        ...
        import pickle
        with open(os.path.join(output_dir, "mass_trace.pkl"), "wb") as f:
            pickle.dump(mass_trace, f)
        logger.info(f"Saved MASS trace to {output_dir}/mass_trace.pkl")
        
        if final_avg_k is not None:
            avg_k_filename = os.path.join(output_dir, f"Final_K_{final_K}_Avg_k_{final_avg_k:.2f}.txt")
    ...
    return best_eval_metric, best_metric, final_K, final_avg_k
...
if __name__ == "__main__":
    ...
    if args.save_model:
        args.output_dir = os.path.join('./results', args.model_name_or_path+"_mass", args.task_name + (f'/{args.output_dir}' if args.output_dir else ''))
    else:
        args.output_dir = os.path.join('./results', args.model_name_or_path+"_mass", args.task_name + (f'/{args.output_dir}' if args.output_dir else ''))
    ...
    if args.to_MoE:
        args.output_dir = args.output_dir + f'/MoE_{args.moe_layers}_experts_{args.num_experts}_adaptive_topk_key_gate_{args.key_gate}_{args.gate_type}_aux_{args.aux_loss_weight}_noise_{args.gate_noise}_capacity_{args.capacity_factor}'
    
    if args.to_MoE and hasattr(args, 'expansion_start_ratio') and hasattr(args, 'expansion_end_ratio'):
        if args.expansion_start_ratio != 0.0 or args.expansion_end_ratio != 0.1: 
            args.output_dir = args.output_dir + f'_expand_{args.expansion_start_ratio:.2f}to{args.expansion_end_ratio:.2f}'
    ...
    best_results = 0
    best_lr = 0
    best_final_K = args.num_experts  # Initialize with initial K
    best_final_K_std = 0.0
    best_final_avg_k = 0.0
    best_final_avg_k_std = 0.0
    for lr in args.learning_rates:
        ...
        for seed in args.seeds:
            best_eval_metric, best_metric, final_K, final_avg_k = main(args, seed, lr)
            torch.cuda.empty_cache()
            temp_results.append(best_metric*100)
            temp_final_K.append(final_K)
            temp_final_avg_k.append(final_avg_k)
            all_results[f'lr{lr}_s{seed}'] = best_eval_metric
        results[f'lr{lr}'] = np.mean(temp_results)
        results[f'lr{lr}_std'] = np.std(temp_results)
        final_K_results[f'lr{lr}_K'] = np.mean(temp_final_K)
        final_K_results[f'lr{lr}_K_std'] = np.std(temp_final_K)
        final_avg_k_results[f'lr{lr}_avg_k'] = np.mean(temp_final_avg_k)
        final_avg_k_results[f'lr{lr}_avg_k_std'] = np.std(temp_final_avg_k)
        if results[f'lr{lr}'] > best_results:
            best_results = results[f'lr{lr}']
            best_results_std = results[f'lr{lr}_std']
            best_lr = lr
            best_final_K = final_K_results[f'lr{lr}_K']
            best_final_K_std = final_K_results[f'lr{lr}_K_std']
            best_final_avg_k = final_avg_k_results[f'lr{lr}_avg_k']
            best_final_avg_k_std = final_avg_k_results[f'lr{lr}_avg_k_std']
    ...
    if args.output_dir is not None:
        with open(os.path.join(args.output_dir, "all_results.json"), "w") as f:
            ...
            f.write('\n')
            if args.to_MoE and args.enable_mass:
                json.dump(final_K_results, f, indent=2)
                f.write('\n')
                json.dump(final_avg_k_results, f, indent=2)
                f.write('\n')
        with open(os.path.join(args.output_dir, f"best_lr_{best_lr}_{best_results}_{best_results_std}.txt"), "w") as f:
            ...
            f.write(f'best_final_K: {best_final_K}+std_{best_final_K_std}')
            f.write(f'best_final_avg_k: {best_final_avg_k}+std_{best_final_avg_k_std}')
            
        if best_final_K is not None:
            avg_k_filename = os.path.join(args.output_dir, f"{args.seeds}_{args.learning_rates}_{best_results:.3f}+{best_results_std:.3f}_best_final_K_{best_final_K:.3f}+std_{best_final_K_std:.3f}_best_final_avg_k_{best_final_avg_k:.3f}+std_{best_final_avg_k_std:.3f}.txt")
            with open(avg_k_filename, "w") as f:
                f.write(f'best_final_K: {best_final_K}\n')
                f.write(f'best_final_avg_k: {best_final_avg_k}\n')
                f.write(f'best_final_avg_k_std: {best_final_avg_k_std}\n')
            
            print(f"Final Average Top-K: {best_final_K}+std_{best_final_K_std}")
            print(f"Final Average k: {best_final_avg_k}+std_{best_final_avg_k_std}")