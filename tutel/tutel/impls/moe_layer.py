...
class MOELayer(torch.nn.Module):
    ...
    def forward(self, input: Tensor, gate_index=0, capacity_factor=None, top_k=None, a2a_ffn_overlap_degree=None, reserve_dims=1, inequivalent_tokens=False, adaptive_r=None):
        ...
        def routing(top_k):
            ...
            num_experts = self._num_global_experts.data.item()
            _loss_fn = None
            if self.is_gshard_loss:
                ...
                def safe_gather(logits, topk_ids, num_experts):
                    safe_ids = topk_ids.clone()
                    mask = (safe_ids >= num_experts)
                    safe_ids[mask] = 0 
                    gathered = logits[:, :num_experts].gather(index=safe_ids, dim=1)
                    gathered[mask] = 0 
                    return gathered

                _loss_fn = lambda gates, topk_ids: losses.load_importance_loss(
                    F.softmax(logits[:, :num_experts], dim=1), 
                    safe_gather(logits_w_noise, topk_ids, num_experts),
                    self.num_global_experts, gctx.gate_noise)

            elif hasattr(gctx, 'sim_matrix'):
