...
def extract_critical(num_experts, scores, top_k, loss_fn=losses.gshard_loss, capacity_factor=1.0, batch_prioritized_routing=False, normalize_gate=True, alignment=1, group=None, inequivalent_tokens=False, one_score_gate=False):
    ...
    topk_indices = torch.nn.utils.rnn.pad_sequence(topk_indices, True, padding_value=num_experts)
    ...
    if capacity_factor > 0: 
        capacity = max_top_k * int(capacity_factor * samples_per_expert) 
    else:
