...
def print_results_tables(records, selection_method, latex):
    ...
    preferred_order = ["PACS", "VLCS", "OfficeHome", "TerraIncognita", "DomainNet"]
    dataset_names = Q(records).select("args.dataset").unique()
    dataset_names = [d for d in preferred_order if d in dataset_names]
    ...
    for dataset in dataset_names:
        ...
        for i, algorithm in enumerate(alg_names):
            ...
            for j, test_env in enumerate(test_envs):
                grouped = grouped_records.filter_equals(
                        "dataset, algorithm, test_env",
                        (dataset, algorithm, test_env))
                trial_accs = (grouped).select("sweep_acc")
...
if __name__ == "__main__":
    ...
    if args.all:
        records = reporting.load_records_all(args.input_dir, seed=args.seed)
    elif args.data:
        records = reporting.load_records_data(args.input_dir, seed=args.seed)
    else:
        records = reporting.load_records(args.input_dir, seed=args.seed)
