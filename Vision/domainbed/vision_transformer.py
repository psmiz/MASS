...
from domainbed.vit_helpers import build_model_with_cfg, named_apply, adapt_input_conv
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, 
                 ...
                 cur_depth=0, moe_layers=None, num_experts=6, 
                 router='cosine_top', is_tutel=False, index_hook=None,
                 topk=1, **kwargs):
        super().__init__()
        ...
        self.MoE_from_ffn = kwargs.get('MoE_from_ffn', False)
        if self.cur_layer == 'S':
            self.num_experts = num_experts
            if self.MoE_from_ffn:
                self.topk = topk
                self.random_init_gate =  kwargs.get('random_init_gate', False)
                assert self.mlp_hidden_dim % num_experts == 0
                self.mlp_hidden_dim = self.mlp_hidden_dim // num_experts
                print("split FFN to construct MoE!")
            self.expert_repeat = kwargs.get('expert_repeat', 1)
            if self.expert_repeat > 1:
                assert self.MoE_from_ffn, "expert repeat only works for MoE from FFN"
                print("repeat the expert for {} times".format(self.expert_repeat))
                self.mlp_hidden_dim *= self.expert_repeat
            # print(f'cur_layer {cur_depth} is sparse with {num_experts} experts with BPR True')
            print("gate parameters: type {} noise {} capacity {}".format(router, kwargs.get('gate_noise', 1.0), kwargs.get('capacity_factor', 1.5)))
            if router == 'gated_multi_gate':
                    self.mlp = tutel_moe.moe_layer(
                        gate_type={'type': 'gated_multi_gate', 'max_expert_num': kwargs.get('max_expert_num', 8), 'fp32_gate': True, 'adaptive_experts': kwargs.get('adaptive_experts', False)},
                        experts={'type': 'ffn', 'count_per_node': num_experts,
                            'hidden_size_per_expert': self.mlp_hidden_dim,
                            'activation_fn': lambda x: self.moe_drop(F.gelu(x)),
                            },
                        model_dim=dim,
                        batch_prioritized_routing=True,
                        is_gshard_loss=kwargs.get('is_gshard_loss', True),
                ...
                self.mlp = moe_layer_mass(
                    gate_type={'type': 'mintau', 
                               'max_expert_num': kwargs.get('max_expert_num', 8), 
                               'fp32_gate': True, 
                               'adaptive_experts': True,
                               'rm_threshold': kwargs.get('rm_threshold', 1.0)},
                    experts={'type': 'ffn', 'count_per_node': num_experts,
                        'hidden_size_per_expert': self.mlp_hidden_dim,
                        'activation_fn': lambda x: self.moe_drop(F.gelu(x)),
                    },
                ...
                self.mlp = tutel_moe.moe_layer(
                    gate_type={'type': router, 'k': topk, 'fp32_gate': True, 
                            'gate_noise': kwargs.get('gate_noise', 1.0), 
                            'capacity_factor': kwargs.get('capacity_factor', 1.5)},
                    experts={'type': 'ffn', 'count_per_node': num_experts,
                            'hidden_size_per_expert': self.mlp_hidden_dim,
                            'activation_fn': lambda x: self.moe_drop(F.gelu(x)),
                            },
                    model_dim=dim,
                    batch_prioritized_routing=True,
                    is_gshard_loss=kwargs.get('is_gshard_loss', False),
                    one_score_gate=kwargs.get('one_score_gate', False),
                    normalize_one_score_gate=kwargs.get('normalize_one_score_gate', False),
                    update_momentum=kwargs.get('one_score_gate_update_momentum', 0.0),
                )
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 router='cosine_top', is_tutel=False, topk=1, **kwargs):
        """
        Args:
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                router=router, is_tutel=is_tutel, topk=topk, **kwargs)
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    for i, block in enumerate(model.blocks.children()):
        ...
        if hasattr(block.mlp, 'experts'):
            num_experts = getattr(block.mlp, 'experts').batched_fc1_w.shape[0]
            fc1_weight = _n2p(w[f'{block_prefix}MlpBlock_3/Dense_0/kernel']).unsqueeze(0).repeat(num_experts, 1, 1)
            fc2_weight = _n2p(w[f'{block_prefix}MlpBlock_3/Dense_1/kernel']).unsqueeze(0).repeat(num_experts, 1, 1).permute(0, 2, 1)
            hidden_dim, input_dim = fc1_weight.shape[1], fc1_weight.shape[2]
            if block.MoE_from_ffn: 
                pass
            else:
                getattr(block.mlp.experts, f'batched_fc1_w').copy_(fc1_weight)
                getattr(block.mlp.experts, f'batched_fc2_w').copy_(fc2_weight)
                
                fc1_bias = _n2p(w[f'{block_prefix}MlpBlock_3/Dense_0/bias']).unsqueeze(0).repeat(num_experts, 1)
                fc2_bias = _n2p(w[f'{block_prefix}MlpBlock_3/Dense_1/bias']).unsqueeze(0).repeat(num_experts, 1)
                getattr(block.mlp.experts, f'batched_fc1_bias').copy_(fc1_bias)
                getattr(block.mlp.experts, f'batched_fc2_bias').copy_(fc2_bias)
        else:
            for r in range(2):
